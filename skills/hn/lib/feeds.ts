...
export function parseRSSItems(xml: string): Array<{ title: string; link: string; pubDate: string; description: string }> {
  ...
  if (isAtom) {
    ...
    while ((itemMatch = itemPattern.exec(xml)) !== null) {
      ...
      const pubDate =
        getTagContent(itemXml, "pubDate") || getTagContent(itemXml, "dc:date") || getTagContent(itemXml, "date");
...
async function fetchFeed(
  ...
  try {
    ...
    log.warn(`✗ ${feed.name}: ${msg.includes("abort") ? "timeout" : msg}`);
    return [];
  }
}
...
export async function fetchAllFeeds(
  ...
  for (let i = 0; i < feeds.length; i += FEED_CONCURRENCY) {
    ...
    const results = await Promise.allSettled(batch.map((feed) => fetchFeed(feed, log)));
    ...
    log.info(`Progress: ${progress}/${feeds.length} feeds processed (${successCount} ok, ${failCount} failed)`);
  }

  log.info(`Fetched ${allArticles.length} articles from ${successCount} feeds (${failCount} failed)`);
  return allArticles;
}
